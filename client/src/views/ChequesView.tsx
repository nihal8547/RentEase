...
export default function ChequesView() {
  ...
  const createMutation = useMutation({
    ...
    onError: (err: any) => {
      setFormError(err.response?.data?.message || 'Failed to create cheque. Please verify your entries.');
    },
  });
  ...
  const cheques = (data || []).filter(c =>
    (filterStatus === 'ALL' || c.status === filterStatus) &&
    (filterBank === 'All Banks' || c.bankName === filterBank)
  );

  const stats = {
    total:    data?.length || 0,
    cleared:  data?.filter(c => c.status === 'CLEARED').length || 0,
    pending:  data?.filter(c => c.status === 'PENDING' || c.status === 'IN_VAULT').length || 0,
    bounced:  data?.filter(c => c.status === 'BOUNCED').length || 0,
    totalVal: data?.filter(c => c.status === 'PENDING' || c.status === 'IN_VAULT').reduce((s, c) => s + Number(c.amount), 0) || 0,
  };
  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: 'Total Cheques',      val: stats.total,   color: 'text-[#221E1C]',  bg: 'bg-white' },
          { label: 'Cleared',            val: stats.cleared, color: 'text-emerald-700', bg: 'bg-emerald-50' },
          { label: 'Pending / In Vault', val: stats.pending, color: 'text-amber-700',  bg: 'bg-amber-50' },
          { label: 'Bounced',            val: stats.bounced, color: 'text-red-700',    bg: 'bg-red-50' },
        ].map(s => (
      ...
      <div className="bg-[#FAF6EE] border border-[#E4DCCB] rounded-xl p-4 flex items-center gap-3 shadow-sm">
        ...
        <span className="ml-auto rtl:mr-auto text-xl font-bold text-[#6E1731]">{formatQAR(stats.totalVal)}</span>
      </div>
      ...
      <div className="bg-white border border-[#E4DCCB] rounded-2xl overflow-hidden shadow-sm">
        {isLoading ? (
          <div className="flex items-center justify-center h-48 text-[#8B8279] text-sm">Loading cheques…</div>
        ) : cheques.length === 0 ? (
          <div className="p-8">
            <EmptyState
              icon={Landmark}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[#E4DCCB] bg-[#FBF9F3]">
                  ...
                  <th className="py-3.5 px-4 text-left text-xs font-semibold text-[#5B534C] uppercase tracking-wider">Cheque #</th>
                  <th className="py-3.5 px-4 text-left text-xs font-semibold text-[#5B534C] uppercase tracking-wider">Tenant / Drawer</th>
                  <th className="py-3.5 px-4 text-left text-xs font-semibold text-[#5B534C] uppercase tracking-wider">Bank</th>
                  <th className="py-3.5 px-4 text-left text-xs font-semibold text-[#5B534C] uppercase tracking-wider">Amount</th>
                  <th className="py-3.5 px-4 text-left text-xs font-semibold text-[#5B534C] uppercase tracking-wider">Due Date</th>
              ...
              <tbody className="divide-y divide-[#E4DCCB]">
                {cheques.map((c) => {
                  ...
                  const Icon = cfg.icon;
                  return (
                    <tr key={c.id} className="hover:bg-[#FBF9F3]/70 transition-colors">
